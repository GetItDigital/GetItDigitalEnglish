...
\begin{frame}{}
	...
	\ftx{Learning objectives: Simple direct current networks}
...
\begin{frame}
	\ftx{Series connection of resistors}
	...
	\s{
		...
		\begin{equation}
			U_\mathrm{tot} = \sum_{k=1}^{n} U_k = \sum_{k=1}^{n} R_k \cdot I = R_\mathrm{tot} \cdot I
		...
		\begin{Key point}{Total conductance of a series connection}{}
			...
			\begin{equation}
				\frac{1}{G_\mathrm{tot}} = \sum_{k=1}^{n} \frac{1}{G_k}
			\end{equation}
			%	\begin{equation}
	...
	\b{
		...
		\begin{columns}
			...
			\begin{tikzpicture}
				...
				to[R ,i ,v< , name =R1  , l =$R_\mathrm{tot}$] (3,2.5)
				to[short](3,3) 
				%		to[short](2.5,3) 
				%		to[R ,i ,v< , name =R2  , l =$R_2$]  (0.5,3)
				to[short](0,3)
...
\begin{frame}
	\ftx{Parallel connection of resistors}
	...
	\s{
		...
		( $I_0 = U_0 \cdot \frac{1}{R_\mathrm{tot}}$)
		...
		\begin{equation}
			R_\mathrm{tot} = \frac{R_1\cdot R_2}{R_1 + R_2}
			\label{eq:r2parallel}
		\end{equation}
		...
		\begin{Key point}{Total resistance of a parallel connection}
			...
			\begin{equation}
				\frac{1}{R_\mathrm{tot}} = \sum_{k=1}^{n} \frac{1}{R_k}
				\label{eq:rparallel}
			\end{equation}
			%	\begin{equation}
		...
		\begin{expl}{Parallel connection of resistors}{parallelschaltung}
			...
			\begin{equation*}
				R_\mathrm{tot} = \frac{1}{\sum_{n} \frac{1}{R_n}} = \frac{1}{\frac{1}{R_1} + \frac{1}{R_2}+ \frac{1}{R_3}}
			\end{equation*}
			
			Inserting the numerical values and solving using a calculator:
			
			\begin{equation*}
				R_\mathrm{tot} = \frac{1}{\frac{1}{1\cdot 10^3 \, \Omega} + \frac{1}{10\cdot 10^3 \, \Omega} + \frac{1}{100 \cdot 10^3 \, \Omega}}
			\end{equation*}
			
			\begin{equation*}
				\rightarrow R_\mathrm{tot} = 900,9 \, \Omega
			\end{equation*}
	...
	\b{
		\begin{columns}
			...
			%\phantom{.}\\
			Mesh rule: $U_\mathrm{R1} = U_\mathrm{R2} = U_0$\\
			
			Knot rule: $I_0 - I_1 - I_2 = 0$\\
			
			Ohm's law: $I_1 = \frac{U_0}{R_1}$ % $I_2 = \frac{U_0}{R_2}$ 
			...
			\begin{tikzpicture}
				\draw(0,3) to[short] (2.5,3)
				to[R=$R_\mathrm{tot}$, i>^, v, name=Rges] (2.5,0)
				to[short] (0,0);
				\draw(0,3) to[V, v, i, name=U0] (0,0);
...
\begin{frame}
	...
	\s{
		...
		\begin{expl}{Unloaded voltage divider}{Spannungsteiler}
		...
		\begin{expl}{Loaded voltage divider}{Spannungsteiler}
	...
	\b{
		\ftx{Voltage dividers in resistor networks}
		...
		\begin{columns}
			...
			Relationship between total voltage $U_0$ and partial voltage $U_2$:
			...
			\visible<2->{
				...
				\vspace*{-15pt}
				Only applies to \textbf{unloaded voltage dividers}
				(load current = 0)!
				
				Load case: Combine $R_2$ and $R_\mathrm{L}$!
...
\begin{frame}
	...
	\s{
		...
		A parallel connection of two resistors, as shown in Figure \ref{fig:stromteiler}, divides a current $I_0$ flowing in a circuit into two partial currents $I_1$ and $I_2$. Such a circuit is called a current divider.
	...
	\ftx{Current dividers in resistor networks}
...
\begin{frame}
	...
	\ftx{The real source of tension}
	...
	\b{
		...
		\begin{columns}
			...
			Possesses internal resistance $R_\mathrm{i}$\\
...
\begin{frame}
	...
	\s{
		...
		\paragraph{The replacement voltage source}\\
	...
	\b{
		...
		\begin{columns}
			...
			Each \textbf{active} two-pole can be represented as a \textbf{substitute voltage source}\\
			
			\phantom{text}\\
			
			\textbf{Ideal voltage source} $U_0$ and 
			\textbf{Internal resistance} $R_i$
...
\begin{frame}
	\ftx{Special operating conditions of the backup voltage source}
	...
	\b{
		...
		\begin{columns}
			...
			Determination of $U_0$ and $R_\mathrm{i}$ via \textbf{short circuit} and \textbf{no-load test}:
			\vspace*{5pt}
			
			Short circuit: $R_a = 0$, $I = I_\mathrm{K}$ = short-circuit current\\
			...
			\only<2->{
				...
				No load: $R_\mathrm{a} \rightarrow \infty$, $U = U_\mathrm{L} = $ no-load voltage
			...
			\begin{tikzpicture}
				...
				\node at (0.6,1.2) {Short-};
				\node at (0.7,0.8) {circuit-};
				\node at (0.7,0.4) {test};
	...
	\b{
		...
		\begin{columns}
			...
			\only<2->{
				...
				\textbf{Reale} Current source:
				
				Possesses internal resistance $R_\mathrm{i} < \infty \, \Omega$ \\
				\begin{equation*}
					I_\mathrm{Ri} > 0 
				\end{equation*}
			...
			\begin{tikzpicture}
				...
				\node at (1.6,3.3) {Ideal current source};
			...
			\begin{tikzpicture}
				\node at (1.6,3.3) {Real current source};
				\draw[white] (3.75, 4) to[short,v>, name=volti] (3.75, -1.0) node[right] {};
				\varrmore{volti}{};
				\node[voltage] at (3.2,1.4) {$U$};
...
\begin{frame}
	...
	\b{
		...
		\begin{columns}
			...
			Each \textbf{active} two-pole can be represented as a \textbf{replacement current source}\\
			
			\phantom{text}\\
			
			Iideal current source $I_0$ with 
			\textbf{internal resistance} $R_i$
...
\begin{frame}
	\ftx{Special operating conditions of the backup power source}
	...
	\b{
		...
		\begin{columns}
            ...
            Determination of $I_0$ and $R_\mathrm{i}$ via \textbf{short circuit} and \textbf{no-load test}:
			...
			\only<2->{
				...
				Idling: $R_\mathrm{a} \rightarrow \infty$, $U = U_\mathrm{L} = $ Open-circuit voltage
...
\begin{frame}
	...
	\s{
		...
		\textbf{Attention}: The Sign Conventions of current and voltage sources converted into each other are directed in opposite directions!
	...
	\b{
		...
		\ftx{Conversion of voltage and current sources}
		\begin{columns}
			\column[t]{0.6\textwidth}
			%Umwandlung teilweise vorteilhaft
			
			\textbf{Internal resistance} $R_\mathrm{i}$ remains identical,\\
			Circuit changes\\
			
			\phantom{.}\\
			
			Convert \textbf{open-circuit voltage} and \textbf{short-circuit current} to another source:			
			\phantom{text}\\
