...
\begin{tikzpicture}[x=1cm, y=1cm]
    ...
    \draw(Energiespeicher) node[xshift=0.2cm, align=center, right]{Energy-\\storage};

   
    % new Plot
    \draw(8,1.5) node[align=center, scale=1.25]{\underline{Properties}};
    ...
    \onslide<1->{% start overlay
      ...
      \draw(6.25,-0.25) node[align=center, text centered]{Energy-\\storage};% Energiespeicher
      
      % rechts
      \draw[thick,->](7.25,-0.25)--(8.25,-0.25);
      \draw(9,-0.25)node{\underline{ODE}};% --> DGL
      ...
      \draw(6.25,-1.75) node[align=center, text centered]{time-\\invariant};% zeitinvariant

      % rechts
      \draw[thick,->](7.25,-1.75)--(8.25,-1.75);
      \draw(9,-1)--(9,-1.75)--(9.5,-1.75);
      \draw(10.75,-1.75) node[text centered, align=center]{constant \\ Coefficients};% --> konst. Koeffizienten
    }% end overlay
    ...
    \onslide<2->{% start overlay
      ...
      %{\begin{varwidth}{\linewidth}%
        ...
        \begin{tabular}{rl}
          %\toprule
          & \underline{ODE-Sizes} \\[2pt]
          \toprule%\midrule
          $n$     :& Order of ODE \\[2pt]
          $a_i$   :& Const. coefficients \\[2pt]
          $y(t)$  :& Circuit size \\[2pt]
          $b(t)$  :& Distur. var. \\%[2pt]
          \bottomrule
        \end{tabular}
      %\end{varwidth}}
