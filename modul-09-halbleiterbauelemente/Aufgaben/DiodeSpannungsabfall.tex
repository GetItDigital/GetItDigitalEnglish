\subsection{Calculation of the voltage drop across a diode\label{Aufg3}}
% Alt: Aufgabe 3
\Aufgabe{
  Given is a series connection consisting of a DC voltage source, a resistor and a silicon diode with a barrier voltage of $U_\mathrm{S}=0.7\,\mathrm{V}$.
  What is the approximate voltage drop across the resistor $R$ when the supply voltage is $U_0=5\,\mathrm{V}$?
  \begin {figure} [H]
   \centering
...
\Loesung{
  Since this is a silicon diode, the gate voltage and the resulting voltage drop
  across the component are approximately $0.6 \text{ to } 0.7\,\mathrm{V}$.
  
  In series connection, the following relationship applies:
  \begin{equation*}
  ...
  \end{equation*}
  See section: \ref{sec:ElektrischesVerhalten}
