...
\begin{frame}
    ...
    \b{
    \begin{itemize}    
		\item Consideration of a black box as a system with input variables and output variables.
    \end{itemize}
    }
    ...
    \b{
    \begin{itemize}    
		\item Transformation of the input signal $F_\mathrm{E}$ into an output signal $F_\mathrm{A}$.
    \end{itemize}
    }
...
\begin{frame}
    ...
    \b{
        \begin{itemize}    
            \item  Description of a black box as an electrical network with an input voltage
    ...
    \b{
        \begin{itemize}    
            \item Transformation of the input voltage $U_\mathrm{E}$ into an output voltage $U_\mathrm{A}$.
        \end{itemize}
    }
...
\begin{frame}
    ...
    \b{
        \begin{itemize}    
            \item<1-> Causal systems do not exhibit a system response prior to excitation.
            \item<2-> Time-invariant systems always respond identically to an input signal, regardless of the timing.
            \item<2-> Linear system $\rightarrow$ finite output amplitude for finite input amplitude
            \item<3-> Time-invariant and linear systems are categorised as LTI systems.
        \end{itemize}
    }
...
\begin{frame}
    ...
    \b{
        Conditions and execution of the overlay set:
        \begin{itemize}
            \item<1-> Assumed linearity
            \item<2-> Switching off the sources
            \item<3-> Ideal voltage source replaced by a short circuit
            \item<4-> Ideal power source is replaced by open terminals, an open circuit
        \end{itemize}
    }
    ...
    \fu{
        ...
        \hspace{0.1cm}
        \begin{minipage}{0.3\textwidth}
            \input{Tikz/src/umwandlungstromquelle3.tex}		
		\end{minipage}
        \hspace{0.1cm}
        }
...
\begin{frame}
    ...
    \begin{Key point}{Conversion of sources}
...
\begin{frame}
    \ftx{Summary of superposition theorem}
    ...
    \b{
        Assumption : 
        \begin{itemize}
            \item The resistor $R$ is supplied by the two sources $Q_1$ and $Q_2$.
            \item Separate calculation of the current through $R$ 
            \item The total current through $R$ is calculated from the sum of the currents from the two sources:
        \end{itemize}
    }
...
\begin{frame}
    ...
    \b{
        Calculation of a sample network:
...
\begin{frame}
    ...
    \begin{Key point}{Superposition principle}
...
\begin{frame}
    \renewcommand{\figurename}{Figure}
    \ftx{Example: Superposition method}
    \begin{expl}{Superposition method}{}
        ...
        \begin{enumerate}
			...
			\only<3>{
				...
				\onslide<3>{
                    ...
                    \end{figure}
					\begin{eqa}
						K_1: I_\mathrm{tot}&=I_1+I_3 \nonumber\\                        
                        I_\mathrm{tot}&=\frac{U_\mathrm{g}}{R_\mathrm{tot}}=\frac{U_\mathrm{g}}{R_\mathrm{i}+\frac{R_1\cdot(R_2+R_3)}{R_1+R_2+R_3}} \nonumber\\
                        \frac{I_3}{I_\mathrm{tot}}&=\frac{R_1}{R_1+R_2+R_3} \nonumber\\
                        I_3(U_\mathrm{g})&=I_\mathrm{tot}\cdot\frac{R_1}{R_1+R_2+R_3}=\frac{U_\mathrm{g}}{R_\mathrm{i}+\frac{R_1\cdot(R_2+R_3)}{R_1+R_2+R_3}}\cdot\frac{R_1}{R_1+R_2+R_3}\nonumber
					\end{eqa}
				}
			}
			\only<4>{
				...
				\onslide<4>{
                    ...
                    \end{figure}
					\begin{eqa}
						K_2: I_\mathrm{tot}&=I_\mathrm{g}=I_2-I_3 \nonumber\\                        
                        \frac{-I_3}{I_\mathrm{tot}}&=\frac{R_2}{(R_1||R_\mathrm{i})+R_2+R_3} \nonumber\\
