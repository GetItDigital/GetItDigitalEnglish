\begin{tikzpicture}
    \draw (0,0)
    to[short, o-o] (3,0)node[above]{B}
    to[R,l=$r_\mathrm{BE}$] (5,0) 
    to[short, -o] (9,0)node[above]{E};
    
    \draw (0,-2)
    to[short, o-*] (5,-2) node[ground]{}
    to[short, -o] (9,-2)node[below]{C};
    
    \draw (9,0) to[open, v^, name=ua] (9,-2);
    
    \draw (1,0) to[R,l=$R_\mathrm{1}$, *-*](1,-2);
    \draw (2,0) to[R,l=$R_\mathrm{2}$, *-*](2,-2);
    \draw (5,0) to[R,l=$R_\mathrm{3}$, *-*](5,-2);
    \draw (7.5,0) to[I, i^<, *-*, name=ib] (7.5,-2);
    \draw (6,0) to[R,l=$r_\mathrm{CE}$, *-*](6,-2);
    
    \draw (0,0) to[open, v>, name=ue] (0,-2);
    
    \varrmore{ua}{$u_\mathrm{a}$};
    \varrmore{ue}{$u_\mathrm{e}$};
    \iarrmore{ib}{$\mathrm{B}\cdot i_\mathrm{B}$};
\end{tikzpicture}