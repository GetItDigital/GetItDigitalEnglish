...
\begin{figure}[H]
	\centering
	    \b{\includesvg[width=0.5\textwidth]{Ball.svg}}
		\s{\includesvg[width=0.5\textwidth]{Ball.svg}  
...
\speech{Der Energieerhaltungssatz}{1}{
	In diesem Kapitel lernen die Studierenden die wesentlichen Konzepte des Energieerhaltungssatzes.
	Inhalte sind die Erhaltungsgrößen, die Energieformen und die Tatsache, dass Energie weder vernichtet noch erschaffen werden kann.
	}
...
\s{
	...
	Kondensatoren können viele Ladezyklen durchlaufen, haben jedoch eine niedrige Energiedichte. Um die Energie in einer Spule zu speichern, muss durchgehend Strom durch diese fließen, was technisch herausfordernd und nur in Spezialfällen möglich ist.%"in Spezialfällen möglich ist" (sicher dass der Satz so gemeint war?)
		...
		\s{\begin{figure}[H]
			...
			\hspace{-2cm}
				\includesvg[width=1.4\textwidth]{Idealer_Plattenkondensator.svg}
			\end{minipage}
			\begin{minipage}[H]{0.45\textwidth}
				\hspace{-1cm}
				\includesvg[width=1.25\textwidth]{Magnetisches_Feld_Spule.svg}
		  \end{minipage}
...
\begin{frame}
	...
	\b{\begin{figure}[H]
		...
		\begin{minipage}[H]{0.45\textwidth}
			\hspace{-3.05cm}
			\includesvg[width=2\textwidth]{Spule.svg}
		\end{minipage}
...
\begin{frame}
	...
	\begin{figure}[H] % Hier vielleicht Beschriftung oberhalb der Batterie anbringen, damit die Bildunterschrift besser zur Geltung kommt.
		\centering
					\b{\includesvg[width=1\textwidth]{Chemischer_Speicher.svg}}
					\s{\includesvg[width=1\textwidth]{Chemischer_Speicher.svg}
