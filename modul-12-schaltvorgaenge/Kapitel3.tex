...
\s{% skript only, tau
    ...
    \end{align}
    und der exponentielle Abfall von $i(t)$ respektive $u_R(t)$ ab deren Maximalwerten $U_q/R$ und $U_q$:
    \begin{align}
...
&\text{hom. Lsg.}:&
    \ecv{u_{C,h}} &= K \cdot \mathrm{e}^{-\sfrac{t}{\tau}}&
        &\text{mit}\quad \tau = RC
\end{align*}
\textbf{3.) Part. Lsg., 4.) Überlagerung:} (eingeschwungen Null)
\begin{align*}
...
\end{align}
$K_1$ und $K_2$ müssen komplex konjugiert sein, damit $\ecv{u_{C,\mathrm{h}}}$ reell ist! \s{\cite[Vgl. S. 379]{hagmann}}
\begin{align}
