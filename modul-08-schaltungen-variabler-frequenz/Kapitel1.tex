...
\b{
    \begin{columns}[T]
        \column{0.45\textwidth}% column links 1/2
            ...
            \onslide<5->{\resizebox{\textwidth}{!}{\includegraphics{Tikz/pdf/plot_tiefpass_mini_ampli_lin_samesize.pdf}}}

            \onslide<5->{\resizebox{\textwidth}{!}{\includegraphics{Tikz/pdf/plot_tiefpass_mini_phase_lin_samesize.pdf}}}
            }%
        \column{0.55\textwidth}% column rechts 2/2
            ...
            \begin{align}
                ...
                \onslide<2->{\varphi(\omega)&=\arctan\left(-\omega CR\right)                                        &&\\}%
                \onslide<4->{%
    ...
    \resizeboxsb{1}{1}{% 90% textwidth skript, 100% textwidth beamer
        \begin{tabular}{|c|c l|c l|c|}
            ...
            \\\hline\parbox[c][2cm][c]{0.3cm}{\rotatebox[origin=c]{90}{\textbf{RC}}}
                ...
                &   \parbox[c][2cm][c]{1.7cm}{$\dfrac{1}{1+\mathrm{j}\omega RC}$}
                &   \parbox[c][2cm][c]{3.0cm}{\resizebox{\linewidth}{!}{\includegraphics{Tikz/pdf/circ_hochpass_rc_annotated.pdf}}}
                &   \parbox[c][2cm][c]{1.7cm}{$\dfrac{1}{1-\mathrm{j}\frac{1}{\omega RC}}$}
                &   \parbox[c][2cm][c]{0.5cm}{\centering $\dfrac{1}{RC}$}
            \\\hline
        \end{tabular}
    }%
\end{table}
...
\s{%
    ...
    Die Frequenzgänge der RL- und RC-Varianten unterscheiden sich nur im Faktor $\dfrac{L}{R}$ und $RC$ vor $\omega$ voneinander.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%% ADDITIONAL STUFF AND CODE DUMP --- OUT-COMMENTED / OR SKRIPT & BEAMERONLY -> NOT COMPILED   %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\s{\b{ % Ansatz eins, Zweipol mit Rechteck manuell gezeichnet
\section{testframe}
\begin{frame}\ftx{\secname}
    \fu{ % Schaltsymbole: Zweipol und Vierpol
        \begin{subfigure}{0.48\textwidth}\centering
            \begin{tikzpicture}
                \draw[draw=black](-2,-1.25) rectangle (+2,+1.25); %invisible boundbox
                % Zweipol
                \node[draw=none,align=center] {Zweipol\\Eintor}; % linebreak only with align=...
                \draw[thick](-0.75,-0.5) rectangle (0.75,0.5);
                \draw(-0.75,0) to [short, -o, i>=$I$] ++(-0.5,0);
                \draw(+0.75,0) to [short, -o, i>=$I$] ++(0.5,0);
                \draw(-1.25,0.75) to [open, v^=$U$] (1.25,0.75);
            \end{tikzpicture}
        \end{subfigure}\hfill
        \begin{subfigure}{0.48\textwidth}\centering
            \begin{tikzpicture}
                \draw[draw=black](-2,-1.25) rectangle (+2,+1.25); %invisible boundbox
                % Vierpol
                %\draw (0,0) node{Vierpol\\Zweitor}; % might work also with node's "text width=3cm" option
                %\draw (0,0) node[text width=3cm]{Vierpol Zweipol}; % also doesnt work maybe smaller ... crap
                \node[draw=none,align=left] {Vierpol\\Zweitor};
                \draw(0,0) node[dipchip, hide numbers, num pins=6, no topmark, external pins width=0](C){}; % Mittelpunkt des Vierpols
                \draw(C.bpin 1) to[short,-o, i_<=$I_1$] ++(-0.5,0) coordinate(In1);
                \draw(C.bpin 3) to[short,-o, i_>=$I_1$] ++(-0.5,0) coordinate(In2);
                \draw(C.bpin 6) to[short,-o, i>=$I_2$] ++(0.5,0) coordinate(Out1);
                \draw(C.bpin 4) to[short,-o, i<=$I_2$] ++(0.5,0) coordinate(Out2);
                \draw(In1) to[open, v=$U_1$] (In2);
                \draw(Out1) to[open, v^=$U_2$] (Out2);
            \end{tikzpicture}
        \end{subfigure}
    }{Vergleich: Schaltsymbole eines allgemeinen Zwei- und Vierpols\label{fig:vgl:zweipolvierpol:DEBUG}}	
\end{frame}
}}% end outcommented
