\subsection{Voltage curve in a bridge rectifier circuit\label{Aufg9}}
% Alt: Aufgabe 9
\Aufgabe{
  Sketch the voltage $U_1$ of the given circuit.
  Note that $u_\mathrm{E}$ is an alternating voltage.
  \begin {figure} [H]
     \centering
     \input{Tikz/src/FigDiodeSpannugsverlauf.tex}
...
\Loesung{
  \begin{itemize}
    \item In a negative half-wave, the current flows via $D_{\text{2}}$ and $D_{\text{3}}$.
  \end{itemize}
  \begin{figure}[H]
    \centering
  ...
  \begin{itemize}
    \item In a positive half-wave, the current flows through $D_{\text{4}}$ and $D_{\text{1}}$.
  \end{itemize}
  \begin{figure}[H]
    \centering
...
% }
See section: \ref{sec:Brückengleichrichter}
}
