% Entladevorgang RC Strom und Spannung
...
\begin{tikzpicture}[x=1cm,y=1cm]
	\begin{axis}[
		...
		\addplot[color=black,dashed,	domain=0:5.4] {1};          			% u_max Grenze
		\addplot[color=voltage,very thick,	domain=-1:0.016] {1};           	% t<0
	...
	\begin{axis}[
		...
		\node at (2,-1)[black,anchor=south west,yshift=-1pt]{$-U_q/R$};		% i_min label
		\addplot[color=black,dashed,	domain=-1:5.4] {-1};          		% i_min Grenze
        \addplot[color=red,very thick,	domain=-1:0] {0};  					% t<0
        \addplot[color=red, very thick] coordinates {(0,-1)(0,0)}; 			% Sprung
        \addplot[color=red,very thick,	domain=0:5.4,samples=61] {-e^(-x)}; % t>0
		\node at (1,-0.6)[red,anchor=west]{$i$}; 							% i label
	\end{axis}
\end{tikzpicture}