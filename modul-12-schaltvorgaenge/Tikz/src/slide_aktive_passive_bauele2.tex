...
\begin{tikzpicture}
    ...
    \draw(1, 1.5) node [align=center, scale= 1.25]{\underline{Circuit}};
    ...
    \draw(-0.8, -1.25) node[left]{passive};
    % aktiv (unten, label links)  
    \draw(-0.6,-4.4) node{$\left\{\begin{array}{rcl}\\ \\ \\ \\ \\\end{array}\right.$};
    \draw(-0.8, -4.4) node[left]{active};
    % Energie-speicher (oben, label rechts)
    \onslide<3->{% start overlay
        \draw(2.6,-1.35) node[rotate=180](Energy storage){$\left\{\begin{array}{rcl}\\ \\ \\ \\\end{array}\right.$};
        \draw(Energy storage) node[xshift=0.2cm, align=center, right]{Energy -\\ storage};
    }% end overlay

    % new Plot
    \onslide<2->{% start overlay
        \draw(8,1.5) node[ align=center, scale=1.25]{\underline{Equations}};

        \draw(8,0.5) node[left]{\underline{Components}};
        \draw(8,-0.25) node[left]{Resistance};
        \draw(8,-0.25) node[right]{$\ecv{u_R}=R \cdot \eci{i_R}$};
        \draw(8,-1) node[left]{Inductance};
        \draw(8,-1) node[right]{$\ecv{u_L} = L \cdot \frac{\mathrm{d}}{\mathrm{d}t}\cdot \eci{i_L}$};
        \draw(8,-1.75) node[left]{Capacity};
        \draw(8,-1.75) node[right]{$\eci{i_C} = C \cdot \frac{\mathrm{d}}{\mathrm{d}t} \cdot \ecv{u_C}$};

        \draw(8,-3) node[left]{\underline{Kirchhoff}};
        \draw(8,-3.75) node [left]{Knot:};
        \draw(8,-3.75) node [right]{$\displaystyle{\sum_{}^{} \eci{i} = 0} \vphantom{\sum_{}^{}}$};

        \draw(8,-4.5) node[left]{Mesh:};
        ...
        \draw[GETgreen](9.3,-2.5)node[align=center]{Energy storage};
        \draw(7,-5.5) node[align=center]{%
            \textbf{Energy storage in circuits}\\
            \textbf{lead to switching operations and DGLs.}
        }; 
    }% end overlay
\end{tikzpicture}