...
\begin{tikzpicture}[x=1mm,y=1mm] % gilt für tikz-coordinaten außerhalb der axis-environment
    \draw[draw=none] (-16,-2) rectangle (120,36); % Bildrahmen, Koordinatenbezug auf (0,0) des \begin{axis}...\end{axis} pgfplots, für
    \begin{loglogaxis}[
        %title={Bandpass},
        ...
        \addplot+[dashed,mark=none,blue,domain=\xomegamin:\omegagh]   { 1/(\omegagh/x) }        % ideal
            coordinate [pos= (0) ] (A) % static
            coordinate [pos= (1/2) ] (B) % static
        ;
        \addplot+[dashed,mark=none,blue,domain=\omegagt:\xomegamax]   { 1/(x/\omegagt) }        % ideal
            coordinate [pos= (1/2) ] (C) % static
            coordinate [pos= (1) ] (D) % static
        ;
        % Steigungsdreiecke
        \draw (A) -| (B) node [pos=0.5,anchor=east,yshift=+12pt]{$\frac{+20\ \mathrm{dB}}{\mathrm{Dek}}$}; % slope 20 dB/Dek
        \draw (C) -| (D) node [pos=0.5,anchor=east,yshift=-12pt]{$\frac{-20\ \mathrm{dB}}{\mathrm{Dek}}$}; % slope 20 dB/Dek
        ...
        \addplot+[dashed,mark=none,black,]  coordinates { (\xomegamin, 1/2^0.5) ( \xomegamax, 1/2^0.5) }   
            ...
            node [pos=0.1666,anchor=north] {Sperrbereich}       % static
            node [pos=0.5,anchor=north]  {Durchlassbereich}     % static
            node [pos=0.666+0.1666,anchor=north] {Sperrbereich} % static
        ;
    \end{loglogaxis}
\end{tikzpicture}