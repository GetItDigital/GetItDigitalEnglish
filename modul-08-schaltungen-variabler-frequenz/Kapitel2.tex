...
\s{%
    ...
    kann das (Dezi-)Bel in linearen Systemen ebenfalls verwendet werden wie in Gleichung \ref{eq:def:dezibel:feld} gezeigt ist.
    Die Umrechnung basiert auf der Proportionalität von $P\sim U^2$ beziehungsweise $P\sim I^2$, woraus sich der Faktor $2$ ergibt.%
}%
	\b{\textbf{Dezibel} Hilfsmaßeinheit zur Kennzeichnung logarithmischer Verhältnisse zweier Größen.}%
	\begin{align}% script only - with numbering
        ...
        &\textrm{Leistungswurzelgr.}(\textrm{z.B. }U,\ I)&
            Q_{\mathrm{(F)}} &= \mathrm{log}\frac{U_2^2}{U_1^2}\ \operatorname{B} = 20\cdot \mathrm{log}\frac{U_1}{U_2}\ \dB &&
            \label{eq:def:dezibel:feld}
    \end{align}%
\s{%
...
% Tabellen Vorlage (U2/U1 in dB), auskommentierte Kurzversion, blanko. (Etwas andere Werte, keine Colorierung, default Zeilenhöhe)
\s{\b{
    \begin{table}[H]\centering
        \caption{Tyische Dezibelwerte (Verstärkung in $\dB$) bei Spannungsverhältnissen}
        \begin{tabular}{ |c|c|c|c|c|c|c|c|c| } 
        \hline
        $\frac{U_2}{U_1}$ & 
            $0,1$       & $\frac{1}{\sqrt{2}}$ 	& $1$ 		& $\sqrt{2}$ 	& $2$ 		& $10$ 		& $20$ 		& $100$ \\
            \hline
        $Q_{\dB}$ &
            $-20\ \dB$  & $-3\ \dB$ 			& $0\ \dB$ 	& $+3\ \dB$ 	& $+6\ \dB$ & $20\ \dB$ & $26\ \dB$	& $40\ \dB$ \\
        \hline
        \end{tabular}
    \end{table}
}}
...
\b{
    ...
    \begin{table}[H]\centering
        \begin{tabular}{ |c|g|c|g|c|g|c|g|c| }
            \hline &&&&&&&&\\[-11pt]
                $\frac{U_2}{U_1}$ &
                $\frac{1}{\sqrt{2}}$    & $1$ 		& $\sqrt{2}$ 	& $2$           & $\sqrt{10}$   & $10$	        & $20$ 		& $100$ \\[+2pt]
            \hline &&&&&&&&\\[-11pt]
                $Q_{\dB}$ &
                $-3\ \dB$ \CMT{\footm}    & $0\ \dB$ 	& $+3\ \dB$\CMT{\footmc}   & $+6\ \dB$\CMT{\footmc}   & $10\ \dB$     & $20\ \dB$     & $26\ \dB$\CMT{\footmc}& $40\ \dB$ \\[+2pt]
            \hline
        \end{tabular}
    \end{table}
}
...
\s{% Skript only, ohne Miniapge
    ...
    \begin{align}
        ...
        A(\omega) &= \frac{1}{\sqrt{1 + \left(\omega_{\mathrm{g}}/\omega\right)^2 }} \label{eq:hochpass:normiert:amplitudengang}\\
        ...
        \begin{minipage}{\dimexpr0.5\textwidth-\Colsep\relax}% rechte Hälfte
            ...
            \onslide<3->{
                ...
                \begin{equation}\begin{aligned}
                    %A(\omega_{\mathrm{g}}) &= \frac{1}{\sqrt{1 + (\omega_{\mathrm{g}} CR)^2}} \overset{!}{=} \frac{1}{\sqrt{2}}\\
...
\s{%
    ...
    %Im Durchlassbereich entspricht die Näherung der Geraden $\varphi(\omega) = 0\ \degree$ bis zum Übergangsbereich eine Dekade unterhalb der Grenzfrequenz.
    %Im Übergangsbereich $\pm 1 \mathrm{Dek}$ um $\omega_{\mathrm{g}}$ fällt die Gerade mit einer Steigung von $-45\ \degree / \mathrm{Dek}$ von $0\ \degree$ auf $-90\ \degree$.
    %Im Sperrbereich außerhalb des Übergangsbereiches entspricht die Gerade $\varphi(\omega) = -90\ \degree$. 
    Die maximale Abweichung der Approximation beträgt  $\pm\ 5.71\degree$ an den Knickstellen bei $\omega_g\pm 1\ \mathrm{Dek}$.

    % TODO: omega_n statt omega/omega_g in beiden Plots und Definition davon früher
    \s{\b{%START AUSKOMMENTIERT
    \begin{equation} % Frühere Version, zweispaltig ohne aligned
        \varphi(\omega) =
            \begin{cases}% Works
                0\ \degree &
                    &\omega < 0.1 \omega_g \\
                -45\ \degree + \frac{-45\ \degree}{\mathrm{Dek}}\frac{\omega}{\omega_g} &
                    & 0.1 \omega_g \leq \omega \leq 10 \omega_g \\
                -90\ \degree &
                        &\omega > 10 \omega_g
            \end{cases}
    \end{equation}
...
\index{Ordnung, (Filter-)}%
    ...
    Vgl: \url{https://wiki.mexle.org/elektronische_schaltungstechnik/6_filterschaltungen_ii\#zusammensetzen_des_bandpass_filter} % # escaped in URL
